...
class AWS:
    ...
    def __init__(self, LogsPath, LogsFileName):
        ...
        try:
            ...
            self.resource = self.MySession.resource(service_name=self.AwsVar["s3"])                                     # Connect to "s3"
            self.LogsList.append(["AWS Connection", f"Resource Connection", datetime.now(), "", "Resource connection Successful", ""])
            self.client = self.MySession.client(service_name=self.AwsVar["s3"])                                         # Connect to server "s3"
    ...
    def download_Files_From_AWS(self, Cloud_FilesPath, Local_FilesPath):
        ...
        try:
            ...
            self.client = self.MySession.client(service_name=self.AwsVar["s3"])                                         # connecting to server - s3
